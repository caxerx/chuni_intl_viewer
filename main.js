...
import musicData from "./chunirec.json";
  ...
  for (const [difficulty, div] of Object.entries(divMap)) {
    if (div) {
      ret.push({
        ...
        playCount: strToNum(
          div.querySelector(".text_n").nextElementSibling.innerText
        ),
      });
    }
  }
...
const fullRecordFetch = async () => {
  ...
  for (const [i, s] of songList.entries()) {
    ...
    ret.push(
      ...parseSongRecordFrag(
        await requestSongRecordFrag(s.idx.value, s.token.value)
      )
    );
  }
...
const fastRecordFetch = async () => {
  ...
  for (const difficulty of Object.values(Difficulty)) {
    ...
    for (const songData of songList) {
      ...
      const scoreStr = songData.querySelector(".text_b")
        ? songData.querySelector(".text_b").innerText
        : null;
      if (title && scoreStr) {
        ret.push({
...
const main = async () => {
  ...
  const isFastFetch = confirm(
    "[chuni-intl-viewer] Do you want to perform a fast fetch ?"
  );
  const recordList = isFastFetch
    ? await fastRecordFetch()
  ...
  resultDiv.appendChild(
    createTextDiv(
      `Maximum Achievable Rating: ${(
        (best30Sum + recordList[0].rating * 10) /
        40
      ).toFixed(2)}`
    )
  );
...
if (window.chuniIntlViewer) {
  alert(
    "[chuni-intl-viewer] Please refresh the page before another new fetch."
  );
} else {
